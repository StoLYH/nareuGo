    ...
    <div class="modal-container" @click.stop>
      ...
      <div class="modal-body">
        ...
        <div v-else class="pickup-info">
          <div class="arrival-status">
            ...
            <h3 class="status-title">나르고가 도착했습니다!</h3>
            <p class="status-subtitle">물건을 로봇에 넣어주세요</p>
          </div>
          ...
          <div class="delivery-card" v-if="deliveryData">
            ...
            <div class="info-grid">
              ...
              <div class="info-row">
                ...
                <span class="info-value">{{ deliveryData.buyerName || '구매자명 없음' }}</span>
              </div>
              <div class="info-row">
          ...
          <div class="instruction-card">
            <div class="instruction-steps">
              ...
              <div class="step">
                ...
                <p class="step-text">물건을 안전하게 넣어주세요</p>
              </div>
              <div class="step">
      ...
      <div class="modal-footer">
        ...
        >
          ...
          <span>{{ loading ? '처리 중...' : '물건 넣기 완료' }}</span>
...
import { confirmSellerPlaced } from '../api/delivery/delivery.js'
...
watch(() => props.isVisible, (newValue) => {
  console.log('🔔 [모달] isVisible 변경됨:', newValue)
  if (newValue) {
    console.log('🔔 [모달] ✅ 모달창 표시되어야 함')
    console.log('🔔 [모달] deliveryData:', props.deliveryData)
  } else {
    console.log('🔔 [모달] ❌ 모달창 숨김')
  }
})

// 컴포넌트 마운트 시 초기 상태 로그
onMounted(() => {
  console.log('🔔 [모달] 컴포넌트 마운트됨')
  console.log('🔔 [모달] 초기 isVisible:', props.isVisible)
  console.log('🔔 [모달] 초기 deliveryData:', props.deliveryData)
})

const closeModal = () => {
  console.log('🔔 [모달] 닫기 버튼 클릭됨')
  console.log('🔔 [모달] loading 상태:', loading.value)

  if (!loading.value) {
    console.log('🔔 [모달] ✅ 로딩 중이 아니라 모달 닫기 처리')
    emit('close')
    console.log('🔔 [모달] close 이벤트 발송 완료')
  } else {
    console.log('🔔 [모달] ❌ 로딩 중이라 닫기 방지')
  }
}

const confirmPickup = async () => {
  console.log('🔔 [모달] 물건 넣기 버튼 클릭됨')
  console.log('🔔 [모달] props.deliveryData:', props.deliveryData)

  if (!props.deliveryData?.deliveryId) {
    console.log('🔔 [모달] ❌ deliveryId가 없음')
    alert('배송 정보가 올바르지 않습니다.')
    return
  }

  console.log('🔔 [모달] ✅ deliveryId 확인:', props.deliveryData.deliveryId)
  loading.value = true
  console.log('🔔 [모달] loading 상태 시작')

  try {
    console.log('📦 [모달] 물건 넣기 확인 중 - deliveryId:', props.deliveryData.deliveryId)

    // 백엔드에 물건 넣기 완료 신호 전송
    console.log('📦 [모달] confirmSellerPlaced API 호출 시작')
    const response = await confirmSellerPlaced(props.deliveryData.deliveryId)

    console.log('✅ [모달] 물건 넣기 확인 성공:', response)

    // 성공 알림
    alert('물건 넣기가 완료되었습니다! 나르고가 배송을 시작합니다.')
    ...
    console.error('❌ 물건 넣기 확인 실패:', error)

    const errorMessage = error.response?.data?.message ||
                        error.message ||
                        '물건 넣기 확인 중 오류가 발생했습니다.'
...
.modal-header {
  ...
  background: linear-gradient(135deg, #4682B4 0%, #6EC6CA 100%);
  color: white;
}
...
.primary-button {
  flex: 2;
  background: linear-gradient(135deg, #4682B4, #6EC6CA);
  color: white;
  border: none;
  padding: 14px 20px;
  border-radius: 12px;
...
.primary-button:hover:not(:disabled) {
  background: linear-gradient(135deg, #5A9BD6, #7FD7DA);
