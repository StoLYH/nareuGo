...
namespace gazebo
{
    ...
    public:
        void Load(physics::ModelPtr model, sdf::ElementPtr sdf) override
        {
            ...
            floorZ_.push_back(1.065); // 2층
            floorZ_.push_back(2.065); // 3층
            floorZ_.push_back(3.065); // 4층
    ...
    private:
        ...
        // lift publish with improved speed control
        void CmdLiftToFloor(int f)
        {
            if (!jLift_) return;

            double targetZ = FloorToZ(f);
            double currentZ = JL();
            double distance = std::fabs(targetZ - currentZ);

            cout << "CmdLiftToFloor: " << f << "층 (z=" << targetZ << ")" << endl;
            cout << "  현재 JL=" << currentZ << ", 거리=" << distance << endl;

            // 점진적 가속을 위한 부드러운 속도 제어 (덜컹거림 방지)
            double maxVelocity = 4.0;
            double minVelocity = 2.5;
            double slowDownDistance = 0.05;

            // 현재 속도를 고려한 점진적 가속
            double currentVelocity = std::fabs(jLift_->GetVelocity(0));
            double targetVelocity = (distance > slowDownDistance) ? maxVelocity : minVelocity;

            // 가속 제한: 현재 속도에서 점진적으로 목표 속도로 증가
            double maxAcceleration = 2.0;  // 최대 가속도 제한 (2.0 m/s²)
            double timeStep = 0.1;  // 제어 주기
            double maxVelocityChange = maxAcceleration * timeStep;

            double velocity;
            if (std::fabs(targetVelocity - currentVelocity) > maxVelocityChange) {
                // 점진적 가속/감속
                velocity = currentVelocity + (targetVelocity > currentVelocity ? maxVelocityChange : -maxVelocityChange);
                cout << "점진적 가속: " << currentVelocity << " → " << velocity << " m/s" << endl;
            } else {
                // 목표 속도에 도달
                velocity = targetVelocity;
            }

            cout << "속도 제어: 거리=" << distance << "m, 속도=" << velocity << "m/s" << endl;

            // Gazebo joint에 직접 속도 설정
            if (distance > zTol_) {  // 오차 범위 밖이면 계속 이동
                double direction = (targetZ > currentZ) ? 1.0 : -1.0;
                jLift_->SetVelocity(0, direction * velocity);

                // 목표 위치도 함께 설정 (위치 제어와 속도 제어 병행)
                std_msgs::msg::Float64 m;
                m.data = targetZ;
                pubLift_->publish(m);
            } else {
                // 목표에 충분히 가까우면 멈춤
                jLift_->SetVelocity(0, 0.0);
            }
        }
        ...
        void Step(double dt)
        {
            ...
            case State::MOVE_TO_PICKUP:
            {
                // 처음 1회 + 0.1초마다 보강 발행 (기존 0.5초 → 0.1초)
                if (!liftCmdLatched_ || std::fmod(tInState_, 0.1) < 1e-3)
                {
                    CmdLiftToFloor(pickupFloor_);
                    liftCmdLatched_ = true;
                }
                // 도착 판정 시간 단축 (기존 0.2초 → 0.05초)
                if (IsLiftAtFloor(pickupFloor_) && tInState_ > 0.05)
                {
                    Transit(State::OPEN_PICKUP_DOORS);
                }
                break;
            ...
            case State::CLOSE_PICKUP_DOORS:
            {
                ...
                // 문이 완전히 닫히고 안정화 시간까지 기다린 후 이동
                if (DoorsClosed() && tInState_ > 0.5)  // 문 닫힘 + 1초 안정화 시간
                {
                    cout << "✅ 문 완전 닫힘 확인 - 목적층으로 이동 시작" << endl;
                    CmdLiftToFloor(targetFloorFromRobot_);
                    Transit(State::MOVE_TO_DEST);
                }
                else if (tInState_ > 3.0)  // 최대 3초 대기 (비상 타임아웃)
                {
                    cout << "⚠️ 문 닫힘 타임아웃 - 강제 이동 시작" << endl;
                    CmdLiftToFloor(targetFloorFromRobot_);
                    Transit(State::MOVE_TO_DEST);
                }
                break;
            ...
            case State::MOVE_TO_DEST:
            {
                // 더 빈번한 명령 전송으로 반응성 향상
                if (std::fmod(tInState_, 0.1) < 1e-3) {
                    CmdLiftToFloor(targetFloorFromRobot_);
                }
                // 도착 판정 시간 단축 (기존 0.2초 → 0.05초)
                if (IsLiftAtFloor(targetFloorFromRobot_) && tInState_ > 0.05)
                {
