...
namespace gazebo
{
    ...
    public:
        void Load(physics::ModelPtr model, sdf::ElementPtr sdf) override
        {
            ...
            floorZ_.push_back(1.05); // 2층
            floorZ_.push_back(2.05); // 3층
            floorZ_.push_back(3.10); // 4층
    ...
    private:
        ...
        void Step(double dt)
        {
            ...
            case State::OPEN_PICKUP_DOORS:
            {
                ...
                if (!doorCmdSent_)
                {
                    CmdAllDoorsOpenAtFloor(pickupFloor_); // 엘리베이터+층 문 open
                    doorCmdSent_ = true;                  // 1회만
                }
                if (DoorsOpened() || tInState_ > 2.0)
                {
                    Transit(State::WAIT_BOARD);
                }
                break;
            }
