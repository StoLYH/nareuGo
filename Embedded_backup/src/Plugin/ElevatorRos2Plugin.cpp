...
namespace gazebo
{
        ...
        void Load(physics::ModelPtr model, sdf::ElementPtr sdf) override
        {
            ...
            updateConn_ = event::Events::ConnectWorldUpdateBegin(
                [this](const gazebo::common::UpdateInfo &info)
                {
                    ...
                    // Lift control (PD Controller)
                    if (jLift_)
                    {
                        double z = jLift_->Position(0); // prismatic position
                        double current_vel = jLift_->GetVelocity(0); // current velocity
                        double e = target_z_ - z; // position error

                        // PD control: P term for position error, D term for velocity damping
                        double v_des = kp_pos_ * e - kd_pos_ * current_vel;
                        v_des = std::clamp(v_des, -vmax_lift_, vmax_lift_);
                        // accel limit
    ...
    private:
        ...
        // PD + rate limit params
        double kp_pos_{2.5};        // lift P gain [1/s] (increased for faster response)
        double kd_pos_{1.0};        // lift D gain [s] (damping to prevent overshoot)
        double vmax_lift_{0.8};     // m/s (slightly increased max velocity)
        double amax_lift_{2.0};     // m/s^2 (increased acceleration)
